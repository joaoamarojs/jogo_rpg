...
@export var throw_starting_height : float = 29.0
...
func _ready() -> void:
	...
	animation_player = throwable.find_child( "AnimatedSprite2D" )
	ground_height = animation_player.position.y
...
func _physics_process( delta: float ) -> void:
	animation_player.position.y += vertical_velocity * delta
	if animation_player.position.y >= ground_height:
		destroy()
...
func player_interact() -> void:
	...
	if picked_up == false:
		...
		PlayerManager.player_boy.held_item.add_child( throwable )
		throwable.position = Vector2.ZERO
		PlayerManager.player_boy.pickup_item( self )
...
func throw() -> void:
	throwable.get_parent().remove_child( throwable )
	PlayerManager.player_boy.get_parent().call_deferred( "add_child", throwable )
	throwable.position = PlayerManager.player_boy.position
	animation_player.position.y = -throw_starting_height
...
func drop() -> void:
	...
	PlayerManager.player_boy.get_parent().call_deferred( "add_child", throwable )
	throwable.position = PlayerManager.player_boy.position
	animation_player.position.y = -50
